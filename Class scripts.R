...
#Histogram of wages in 1961
library(ggplot2)
hist_m <- ggplot(wage_data_1961, aes(x = men_subalterni)) +
  geom_histogram(bins = 15, fill = "gray40", color = "black") +
  theme_classic()

hist_m

#Playing with themes :)
library(ggthemes)             # Load the package
#theme_economist
#theme_stata
#theme_excel


hist_f <- ggplot(wage_data_1961, aes(x = women_subalterne)) +
